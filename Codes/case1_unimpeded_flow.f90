...
program rect_obstruction_case1
  ...
  integer,  parameter :: niter = 200000       ! at least 100000 as required
  ...
  !---------------------- apply Case 1 BCs once --------------------------------
  call apply_bc_case1(psi, vort, nx, ny, is_solid, n1, n2, nw, nt, nh)

  !------------------------- define fluid rectangles ---------------------------
  ! Region I: left of the obstruction (prof's Region I)
  ixA1 = 2
  ixA2 = n1
  jyA1 = 2
  jyA2 = nt + 1

  ! Region II: right of the obstruction (prof's Region II)
  ixB1 = n1 + nw + 2
  ixB2 = n1 + n2 + nw
  jyB1 = 2
  jyB2 = nt + 1

  ! Region III: above the obstruction (prof's Region III)
  ixC1 = 2
  ixC2 = n1 + n2 + nw
  jyC1 = nt + 2
  jyC2 = nt + nh            ! = ny - 1
  ...
  do iter = 1, niter
     ...
     ! 1) Applying outer unimpeded BCs and enforce psi in solid region
     call apply_bc_case1(psi, vort, nx, ny, is_solid, n1, n2, nw, nt, nh)
     ...
     ! Re-enforce psi BCs after psi relaxation
     call apply_bc_case1(psi, vort, nx, ny, is_solid, n1, n2, nw, nt, nh)
     ...
     ! Simple progress display to get the idea
  ...
  write(*,*) 'Finished Case 1: final dpmx, domx = ', dpmx, domx

  !----------------------- output final field (x,y,u,v,psi,vort) --------------
  open(unit=uout, file='case1_field_200K_step10.dat', status='replace', action='write')
    ...
    do j = 1, ny
       do i = 1, nx
          if (is_solid(i,j)) then
             psi(i,j)  = 0.0_dp
             ! vorticity inside solid is not used; I leave it as-is or 0
          end if
       end do
    end do
